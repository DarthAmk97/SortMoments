...
def initialize_face_analyzer(det_size=(640, 640), prefer_gpu=True, model_name="buffalo_l"):
    """
    ...
    Args:
        ...
        prefer_gpu: Whether to prefer GPU execution providers when available.
    ...
    providers = get_execution_providers(prefer_gpu=prefer_gpu)
...
def _detect_faces_with_model(face_model, image_rgb):
    """Run a face detector/embedding model using the common InsightFace-style API."""
    if hasattr(face_model, "get"):
        return face_model.get(image_rgb)
    if callable(face_model):
        return face_model(image_rgb)
    raise TypeError("face_model must be callable or expose a .get(image_rgb) method")


def _embed_face_with_model(embedding_model, image_rgb, face):
    """Run a pluggable embedding model and return a vector for one detected face."""
    if embedding_model is None:
        return getattr(face, "embedding", None)

    if hasattr(embedding_model, "embed"):
        try:
            return embedding_model.embed(image_rgb, face)
        except TypeError:
            return embedding_model.embed(face)

    if callable(embedding_model):
        try:
            return embedding_model(image_rgb, face)
        except TypeError:
            return embedding_model(face)

    raise TypeError("embedding_model must be callable or expose an .embed(...) method")
...
def detect_and_embed_faces(input_folder, output_folder,
                           ...
                           progress_callback=None, face_model=None,
                           embedding_model=None, prefer_gpu=True,
                           model_name="buffalo_l", det_size=(640, 640)):
    """
    ...
    Args:
        ...
        face_model: Optional custom detector with .get(image_rgb) or callable(image_rgb)
        embedding_model: Optional custom embedder with .embed(image_rgb, face) or callable
        prefer_gpu: Whether to prefer GPU execution providers when available
        model_name: InsightFace model pack name
    ...
    # Initialize InsightFace unless the caller supplies a compatible model.
    if face_model is None:
        print("Initializing face detection model...")
        face_model = initialize_face_analyzer(
            det_size=det_size,
            prefer_gpu=prefer_gpu,
            model_name=model_name,
        )
    else:
        print("Using caller-provided face detection model...")
    ...
    for batch_start in range(0, total_images, batch_size):
        ...
        for img_path, (processed_img, original_img, scale) in loaded_images.items():
            ...
            faces = list(_detect_faces_with_model(face_model, img_rgb) or [])

            if embedding_model is not None:
                for face in faces:
                    embedding = _embed_face_with_model(embedding_model, img_rgb, face)
                    if embedding is not None:
                        face.embedding = np.asarray(embedding)
...
def reorganize_by_person(output_folder, input_folder=None, similarity_threshold=0.55, max_profile_angle=45, session_id=None, final_output_folder=None):
    """
    ...
    Args:
        ...
        final_output_folder (str): Optional exact destination for organized photos
    ...
    if final_output_folder:
        processed_folder = os.path.join(final_output_folder, session_id) if session_id else final_output_folder
    elif session_id:
